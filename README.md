...
**本项目不是第一个对 ERC-8004 做实证测量的。** 先说清楚这一点，再说区别在哪。

### 已有的学术研究

[**arXiv 2606.26028《Can Trustless Agents Be Trusted?》**](https://arxiv.org/abs/2606.26028)
（Xiong 等，Imperial College，2026-06-24 投稿 / 07-08 修订）是 ERC-8004 的首个实证研究，
覆盖以太坊、BSC、Base 三条链，数据截至 2026-05-13。**它已经做了端点可达性检查**，
结论是三条链上分别只有 3% / 4% / 15% 的注册暴露了「带至少一个可用服务端点」的合法注册文件。

**本项目在几个维度上扩展了它，而不是推翻它：**

| | arXiv 2606.26028 | 本项目 |
|---|---|---|
| 链 | 3 | **12** |
| 身份数 | 17 万+ | **392,258** |
| 时间维度 | 单次爬取 | **两轮，间隔 ≥48h，逐端点求交集**（持续率 99.6%）|
| 抽样偏差处理 | — | **按主机分层回权**（33.2% → 38.6%）|
| 可达判据 | 存在可用服务端点 | **协议层握手**：A2A 解析 agent card / MCP `initialize` + `tools/list` |
| 产出 | 论文 | **MIT 扫描器 + 数据集 + 承诺复跑日期** |

更值得注意的是**两项独立工作在声誉层上互相印证**：该论文测得三条链 73.5% / 59.2% / 90.6%
的评价者存在协同女巫行为、反馈极少锚定可验证交互；本项目从完全不同的方向测得
**有第三方反馈的身份反而更不容易握手成功**（39.7% vs 全体已探测的 47.6%）。
两条互不依赖的路径指向同一结论，比任何一方单独主张都更有力。
...
1. **BSC 一条链就占 67%**（262,999 个），按约 1,800/天增长。各家公布的总数在
   15 万到 44 万之间，量级一致、口径不同 —— 见上文「与已有工作的关系」。
